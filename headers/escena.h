...
private:
    ...
    Color_RR fondo; // Color_RR inicializa en (0, 0, 0) por defecto

public:
    Escena_RR();
    Color_RR getFondo() { return fondo; }
    Camara_RR getCamara() { return camara; }
...
Escena_RR::Escena_RR()
{
    ...
    ParedPtr paredFondo = std::make_shared<Pared_RR>(
        ...
        Vector(-1, 0, 0),     // Normal hacia adelante
    ...
    ParedPtr paredIzquierda = std::make_shared<Pared_RR>(
        Vector(5, 0, -5),     // Centro de la pared
        Vector(0, 0, 1),      // Normal hacia adelante
        10.0f,                // Ancho de la pared
        10.0f,                // Alto de la pared
        ColorRGB(0, 255, 0)); // Color verde
    ...
    ParedPtr paredDerecha = std::make_shared<Pared_RR>(
        ...
        10.0f,                // Ancho de la pared
        10.0f,                // Alto de la pared
        ColorRGB(0, 0, 255)); // Color azul
    objetos.push_back(paredDerecha);

    ParedPtr paredSuperior = std::make_shared<Pared_RR>(
        Vector(5, 5, 0),        // Centro de la pared
        Vector(0, -1, 0),       // Normal hacia adelante
        10.0f,                  // Ancho de la pared
        10.0f,                  // Alto de la pared
        ColorRGB(255, 255, 0)); // Color amarillo
    ...
    ParedPtr paredInferior = std::make_shared<Pared_RR>(
        ...
        10.0f,                  // Ancho de la pared
        10.0f,                  // Alto de la pared
        ColorRGB(255, 0, 255)); // Color magenta
    ...
    //     Vector(0, 0, 0),        // Centro de la pared
    //     Vector(1, 0, 0),        // Normal hacia adelante
    //     10.0f,                  // Ancho de la pared
    //     10.0f,                  // Alto de la pared
    //     ColorRGB(0, 255, 255)); // Color cian
    ...
    //     Vector(5, 0, 0),          // Centro de la pared
    //     Vector(0, 1, 0),          // Normal hacia adelante
    //     10.0f,                    // Ancho de la pared
    //     10.0f,                    // Alto de la pared
    //     ColorRGB(255, 255, 255)); // Color blanco
